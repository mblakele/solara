...
class TelegramClient:
    ...
    def send_message_sync(self, text: str, max_retries: int = 3) -> bool:
        """Send a message synchronously via requests.

        Thread-safe: uses a synchronous HTTP call instead of aiohttp,
        so it can be called from any thread without an event loop.
        ...
        Returns:
            True on success, False on API rejection or exhausted retries.
        """
        url = f"{self.config.base_url()}/sendMessage"
        payload = {
            "chat_id": self.config.chat_id,
        ...
        for attempt in range(max_retries):
            try:
                ...
                if resp.status_code != 200:
                    logger.warning(
                        "Telegram API returned status %d", resp.status_code,
                    )
                    return False
                data = resp.json()
                if not data.get("ok"):
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        "Telegram send attempt %d/%d failed, "
                        "retrying in %.1fs: %s",
