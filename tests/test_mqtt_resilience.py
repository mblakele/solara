...
class FakeClient:
    ...
    def disconnect(self):
        """Emulate paho: flag the client down and wake the network loop."""
...
def harness(monkeypatch):
    ...
    class RecordingStopEvent:
        """Stand-in for the module stop event that records backoff waits."""
    ...
    monkeypatch.setattr(
        mqtt_telemetry, "_stop_event", RecordingStopEvent(), raising=False
    )
...
class TestSubscriberStopPath:
    """Shutdown must stop the subscriber promptly (single-interrupt exit).

    stop_mqtt_subscriber() has to interrupt the reconnect backoff sleep and
    disconnect an active client so the daemon thread exits within ~1 s of a
    shutdown request instead of finishing its full backoff sleep.
    """

    @pytest.fixture()
    def stop_harness(self, monkeypatch):
        """Patch client/dotfile with fast backoff constants; auto-stop."""
        FakeClient.instances = []
        FakeClient.kill_switch = threading.Event()
        FakeClient.loop_blocker = threading.Event()
        monkeypatch.setattr(
        ...
        # Fast retries make "kept reconnecting after stop" observable fast.
        monkeypatch.setattr(mqtt_telemetry, "_MQTT_RECONNECT_MIN_SECS", 0.05)
        monkeypatch.setattr(mqtt_telemetry, "_MQTT_RECONNECT_MAX_SECS", 0.10)
        ...
        def _wait_for(predicate, timeout=5.0):
            ...
            while time.monotonic() < deadline:
                if predicate():
                    return True
                waiter.wait(timeout=0.02)
            return False

        yield SimpleNamespace(thread=t, wait_for=_wait_for)

        # Teardown: force-exit any surviving loop, then join.
        FakeClient.kill_switch.set()
        FakeClient.loop_blocker.set()
        t.join(timeout=5)

    def test_exits_promptly_when_stopped_during_backoff(self, stop_harness):
        """A stop during the reconnect backoff ends the thread at once."""
        FakeClient.connect_behavior = staticmethod(
            lambda attempt: ConnectionRefusedError("refused")
        )

        assert stop_harness.wait_for(lambda: len(FakeClient.instances) >= 1), (
            "subscriber never attempted an initial connect"
        )

        mqtt_telemetry.stop_mqtt_subscriber()
        stop_harness.thread.join(timeout=2.0)
        assert not stop_harness.thread.is_alive(), (
            "subscriber ignored stop_mqtt_subscriber() during backoff"
        )

    def test_stop_disconnects_active_client(self, stop_harness):
        """A connected subscriber gets client.disconnect() on stop."""
        FakeClient.connect_behavior = staticmethod(lambda attempt: None)

        assert stop_harness.wait_for(mqtt_telemetry.is_connected), (
            "subscriber never became connected"
        )
        ...
        stop_harness.thread.join(timeout=2.0)
        assert not stop_harness.thread.is_alive(), (
            "subscriber kept loop_forever running after stop"
        )
        assert client.disconnect_calls == 1, (
            "stop_mqtt_subscriber() must disconnect the active client"
        )
