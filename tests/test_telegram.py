...
# =============================================================================
# 6. TelegramSender.send_notification_sync
# =============================================================================


class TestSendNotificationSync:
    """Tests for TelegramSender.send_notification_sync()."""

    def test_send_notification_sync_success(self, sample_event):
        """send_notification_sync returns True on success."""
        from telegram_client import TelegramConfig
        ...
        mock_client.send_message_sync = MagicMock(return_value=True)
        object.__setattr__(sender, "_telegram_client", mock_client)

        result = sender.send_notification_sync(sample_event)
        assert result is True
        mock_client.send_message_sync.assert_called_once()

    def test_send_notification_sync_failure(self, sample_event):
        """send_notification_sync returns False on failure."""
        from telegram_client import TelegramConfig
        ...
        mock_client = MagicMock()
        mock_client.send_message_sync = MagicMock(return_value=False)
        object.__setattr__(sender, "_telegram_client", mock_client)

        result = sender.send_notification_sync(sample_event)
        assert result is False

    def test_send_notification_sync_not_configured(self, sample_event):
        """Returns False when sender is not configured."""
        sender = TelegramSender.__new__(TelegramSender)
        sender.config = None  # type: ignore[attr-defined]
    ...
    def test_send_notification_sync_creates_client_if_missing(self, sample_event):
        """Creates TelegramClient on first call if not already created."""
        from telegram_client import TelegramConfig
        ...
        with patch("telegram.TelegramClient", return_value=mock_client):
            result = sender.send_notification_sync(sample_event)
            assert result is True
            mock_client.send_message_sync.assert_called_once()
            assert sender._telegram_client is mock_client
