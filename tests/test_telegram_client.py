...
Covers:
  ...
  - TelegramClient.send_message_sync synchronous send via requests
"""
...
# =============================================================================
# 13. TelegramClient.send_message_sync — synchronous send via requests
# =============================================================================
...
class TestSendMessageSync:
    ...
    def test_sync_success(self, telegram_config):
        ...
        with patch("telegram_client.requests.post") as mock_post:
            ...
            client = TelegramClient(telegram_config)
            result = client.send_message_sync("test message")
            assert result is True
            mock_post.assert_called_once()
            call_url = mock_post.call_args[0][0]
            assert "bot123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11" in call_url
            assert mock_post.call_args.kwargs["json"]["chat_id"] == "-100123456"
            assert mock_post.call_args.kwargs["json"]["text"] == "test message"
            assert mock_post.call_args.kwargs["timeout"] == 30

    def test_sync_retry_on_connection_error(self, telegram_config):
        """Retries on requests.ConnectionError, succeeds on second attempt."""
        with patch("telegram_client.requests.post") as mock_post:
            mock_response = MagicMock()
            mock_response.status_code = 200
            mock_response.json.return_value = {"ok": True, "result": {}}
            mock_post.side_effect = [requests.ConnectionError("reset"), mock_response]

            client = TelegramClient(telegram_config)
            result = client.send_message_sync("test")
            assert result is True
            assert mock_post.call_count == 2

    def test_sync_all_retries_exhausted(self, telegram_config):
        """Returns False after all retries are exhausted."""
        with patch("telegram_client.requests.post") as mock_post:
            mock_post.side_effect = requests.ConnectionError("persistent failure")
    ...
    def test_sync_retry_on_timeout(self, telegram_config):
        """Retries on requests.Timeout."""
        with patch("telegram_client.requests.post") as mock_post:
            mock_response = MagicMock()
            mock_response.status_code = 200
            mock_response.json.return_value = {"ok": True, "result": {}}
            mock_post.side_effect = [requests.Timeout("timed out"), mock_response]
    ...
    def test_sync_no_retry_on_non_200(self, telegram_config):
        """HTTP 4xx/5xx returns False immediately (no retry)."""
        with patch("telegram_client.requests.post") as mock_post:
            mock_response = MagicMock()
    ...
    def test_sync_no_retry_on_api_error(self, telegram_config):
        """API ok=False returns False immediately."""
        with patch("telegram_client.requests.post") as mock_post:
            mock_response = MagicMock()
            mock_response.status_code = 200
    ...
    def test_sync_no_retry_on_other_request_error(self, telegram_config):
        """Non-retryable requests exception returns False immediately."""
        with patch("telegram_client.requests.post") as mock_post:
            mock_post.side_effect = requests.URLRequired("invalid URL")
