...
class TelegramSender:
    ...
    def send_notification_sync(
        ...
        """Send a formatted notification synchronously.

        Uses the synchronous send path (requests-based) so it can be
        called from any thread without an event loop.
        ...
        """
        if not self.is_configured:
            logger.warning("Telegram not configured — skipping sync send")
            return False
        ...
        try:
            ...
            if result:
                logger.info("Telegram sync send successful")
            else:
                logger.warning(
                    "Telegram sync send returned False (API rejection or network error)",
                )
            return result
        except Exception as e:  # pylint: disable=broad-exception-caught
